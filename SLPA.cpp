...
/**
 * brief ：initialize the relationship weight of each neighbored nodes
 * author: 
 * date  : 01/25/2016
 */
void SLPA::initRelationWeightNet()
{
	for(int i=0; i<net->N; i++)
	{
		NODE* node = net->NODES[i];
		vector<int> neighbor_weights;
		for(int neighbor_index=0; neighbor_index<node->numNbs; neighbor_index++)
		{
			NODE* neighbor_node = node->nbList_P[neighbor_index];
			int relation_weight = getNodeRelationWeight(node, neighbor_node);
			neighbor_weights.push_back(relation_weight);
		}
		int label = ceateHistogram_selRandMax(neighbor_weights);
		node->WQueue.push_back(label);
	}
...
/**
 * brief : get the relation weight of two nodes, which is calculated using the number
 *        of same neighors and the number of neighbors who are neighbors.
 * author: 
 * date  : 01/25/2016
 */
int SLPA::getNodeRelationWeight(NODE* src_node, NODE* dst_node)
{
	if((NULL == src_node) || (NULL == dst_node))
	{
		return 0;
	}

	int relation_weight = 0;
	for(int dst_nbr_index=0; dst_nbr_index<dst_node->numNbs; dst_nbr_index++)
	{
		NODE* dst_nbr_node = dst_node->nbList_P[dst_nbr_index];
		for(int src_nbr_index=0; src_nbr_index<src_node->numNbs; src_nbr_index++)
		{
			NODE* src_nbr_node = src_node->nbList_P[src_nbr_index];
			for(int depth_nbr_index=0; depth_nbr_index<src_nbr_node->numNbs; depth_nbr_index++)
			{
				//consider the neighbor nodes of src node and dst node that they are neighbor
				NODE* depth_nbr_node = src_nbr_node->nbList_P[depth_nbr_index];
				if(depth_nbr_node == src_node)
				{
					//ignore the same node
					continue;
				}

				if(depth_nbr_node == dst_nbr_node)
				{
					relation_weight += 5;
				}
			}

			if(dst_nbr_node == src_nbr_node)
			{
				//two nodes have same neighbors
				relation_weight += 10;
			}
		}
	}
